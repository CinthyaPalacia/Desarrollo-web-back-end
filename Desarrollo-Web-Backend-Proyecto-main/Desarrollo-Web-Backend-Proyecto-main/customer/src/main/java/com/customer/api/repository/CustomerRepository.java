...
@Repository public interface CustomerRepository extends JpaRepository<Customer, Integer>
{
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE customer " + "SET name = :name, " + "surname = :surname, " + "date_birth = :date_birth, " + "rfc = :rfc, " +
                   "mail = :mail, " + "address = :address " + "WHERE customer_id = :customer_id AND status = 1", nativeQuery = true)
    Integer updateCustomer(
            @Param("customer_id") Integer customer_id,
            @Param("name") String name,
            @Param("surname") String surname,
            @Param("date_birth") LocalDate date_birth,
            @Param("rfc") String rfc, @Param("mail") String mail, @Param("address") String address);
