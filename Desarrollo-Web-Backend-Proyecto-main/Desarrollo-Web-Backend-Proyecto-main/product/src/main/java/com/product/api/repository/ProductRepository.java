...
@Repository public interface ProductRepository extends JpaRepository<Product, Integer>
{
    ...
    @Query(value = "SELECT p.* FROM product p INNER JOIN " +
                   "category c ON p.category_id=c.category_id WHERE p.category_id = :category_id " + "AND p.status = 1", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE product " + "SET gtin = :gtin, " + "product = :product, " + "description = :description, " + "price = :price, " +
                   "stock = :stock, " + "status = 1, " + "category_id = :category_id " + "WHERE product_id = :product_id",
            nativeQuery = true)
